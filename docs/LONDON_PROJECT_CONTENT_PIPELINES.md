...
| 인터넷신문 2개 | 각 신문 하루 3~10건, RSS·속보 기반 | 일반 WP와 분리. 검증 가능한 RSS/1차 출처가 부족하면 숫자를 채우려고 기사를 만들지 않음 | 기사 URL·출처 감사 |
| Blogspot/Blogger | 활성 대상별 하루 1개 비공개 초안 | WordPress 복붙 금지. 같은 주제라도 독립 작성 | 검수 후 공개 |
| Tistory 5개 | 현행 런타임 계약 유지, 별도 변경 지시 전까지 review-gated | 검색형/정책형 독립 글 | URL 감사 |
| YouTube Playlist 5 | 채널별 2~3일마다 1개 | 기존 파이프라인대로 VPS 제작 후 비공개 업로드 | 이사장 검수 후 공개 |
| YouTube Knowledge 5 | 채널별 2~3일마다 1개 | 대본→음성→영상→썸네일→비공개 업로드 | 이사장 검수 후 공개 |
| 10-Language Survival | 언어별 2~3일마다 1강 | 각 언어 총 50강. 현재 2강까지, 다음 3강. 전부 비공개 업로드 | 이사장 최종 검수 후 공개 |
...
- 현재 완료 기준: Lesson 1~2
- 다음 제작: Lesson 3
- 각 언어별 새 Lesson 1개를 2~3일 간격으로 제작
...
- Playlist 5채널과 Knowledge 5채널 모두 각각 2~3일 간격으로 한 편씩 생산한다.
- 채널 간 동일 시각 충돌을 피한다.
