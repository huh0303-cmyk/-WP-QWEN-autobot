...
## Priority 2 — Blogger has two active processing paths (DUPLICATE-SUSPECT)
Path A:
...
## Priority 3 — WordPress publish/draft semantics require verification (HIGH until proven)
...
## Priority 4 — Workflow cleanup candidates (LOW/MEDIUM)
### `curio-scheduler.yml`
...
## Priority 5 — Measurement architecture already exists but needs canonicalization
Verified workflows:
...
- GSC index inspection-derived counts/coverage.
...
2. Canonicalize situation-room YouTube registry and retire stale dashboard entries.
3. Resolve Blogger duplicate ownership with content/job dedup key.
4. Trace and document WP direct-publish vs draft/private semantics.
5. Clean dead workflow conditionals/names/comments without changing business behavior.
6. Continue full repository normalization audit and measurement work from `CODEX_EXECUTION_QUEUE_2026-08-27.md`.
