...
- Groq(무료, llama-3.3-70b-versatile)로 리라이팅, 1000~1500자 신문 기사체
...
GROQ_API_KEY = "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"
...
def run_daily():
    ...
    for sec_i, section in enumerate(SECTIONS):
        ...
        for art_i, news in enumerate(news_items):
            ...
            raw = call_groq(build_news_prompt(news, section, reporter))
            if not raw:
                continue
            ...
            if score < 80:
                ...
                raw2 = call_groq(build_news_prompt(news, section, reporter))
                if raw2:
