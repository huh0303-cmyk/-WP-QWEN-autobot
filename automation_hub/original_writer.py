...
Language: {language}. Persona: {persona}. Tone: {tone}. Target BODY length: {target_chars} visible non-whitespace characters (exclude spaces, HTML markup, JSON, title, labels and meta_description; write 3-4 complete sections with substantive paragraphs) - plan the number of sections and their depth to fit this budget, and finish every section you start; an oversized draft gets trimmed and can end up cut off mid-thought, so write to the budget rather than over-writing and expecting it to be shortened.
Every reader-visible field must use that language, including the title, meta_description, headings, body, labels, image_queries, and image alt text. If the target language is English: the keyword/topic above and the evidence sources below may themselves be written in Korean (they come from Korean-language media as research leads) - that is expected and fine as background, but you must translate every one of those Korean names, terms, and phrases into natural English yourself before writing. Do not copy a single Hangul character into your output anywhere, including as a parenthetical, a transliteration, or a "quoted" original term - not in the title, not in the body, not in a label. If a Korean proper noun has no fixed English name, describe it in plain English instead of writing it in Hangul.
