#!/usr/bin/env python3
"""Guarded install/activation of the free Newsup theme for The Seoul Journal."""
...
add_action('init', function () {
    if (get_option('sj26_newsup_migration_v1') === 'done') return;
    $slug = 'newsup';
    ...
    if (!$theme->exists()) {
        ...
        if (is_wp_error($info) || empty($info->download_link)) {
            update_option('sj26_newsup_migration_v1', 'theme-api-error'); return;
        }
        $result = (new Theme_Upgrader(new Automatic_Upgrader_Skin()))->install($info->download_link);
        if (is_wp_error($result) || !$result) {
            update_option('sj26_newsup_migration_v1', 'install-error'); return;
        }
    }
    switch_theme($slug);
    update_option('sj26_newsup_migration_v1', get_stylesheet() === $slug ? 'done' : 'activate-error');
}, 1);
"""
...
def main():
    ...
    payload = {"name": "The Seoul Journal Newsup migration v1", "desc": "One-time guarded Newsup activation.",
        "code": PHP, "scope": "global", "active": True, "priority": 1,
        "tags": ["theseouljournal", "newsup", "migration"]}
