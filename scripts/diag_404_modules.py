#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
diag_404_modules.py
26개 사이트(k-health365.com 제외) 대상으로:
1) Rank Math '404-monitor', 'redirections' 모듈을 활성화(on) 시도
2) 활성화 후 실제로 등록되는 REST 라우트를 재조회해서 진짜 사용 가능한
   404/redirection 관련 엔드포인트 경로를 찾아냄
결과는 diag_404_modules_result.json 에 저장.
"""
import os, requests, json, sys

WP_USER = "huh0303@gmail.com"

SITES = [
    ("https://koreamedicaltour.com",   "KOREAMEDICALTOURCOM"),
    ("https://koreainvest365.com",     "KOREAINVEST365COM"),
    ("https://ki-korea.com",           "KIKOREACOM"),
    ("https://koreainsurance365.com",  "KOREAINSURANCE365COM"),
    ("https://kfinance365.com",        "KFINANCE365COM"),
    ("https://koreataxnlaw.com",       "KOREATAXNLAWCOM"),
    ("https://koreacrypto365.com",     "KOREACRYPTO365COM"),
    ("https://krealestate365.com",     "KREALESTATE365COM"),
    ("https://ktech365.com",           "KTECH365COM"),
    ("https://kskin365.com",           "KSKIN365COM"),
    ("https://oliveyoungkorea.com",    "OLIVEYOUNGKOREACOM"),
    ("https://kworld365.com",          "KWORLD365COM"),
    ("https://k-trip365.com",          "KTRIP365COM"),
    ("https://k-visa365.com",          "KVISA365COM"),
    ("https://koreawedding365.com",    "KOREAWEDDING365COM"),
    ("https://kstudy365.com",          "KSTUDY365COM"),
    ("https://studyinkorea365.com",    "STUDYINKOREA365COM"),
    ("https://kieca-korea.org",        "KIECAKOREAORG"),
    ("https://ksa-korea.org",          "KSAKOREAORG"),
    ("https://sis-korea.com",          "SISKOREACOM"),
    ("https://jobkorea365.com",        "JOBKOREA365COM"),
    ("https://jobinkorea365.com",      "JOBINKOREA365COM"),
    ("https://jobkoreaglobal.com",     "JOBKOREAGLOBALCOM"),
    ("https://korea365.org",           "KOREA365ORG"),
    ("https://koreanews365.com",       "KOREANEWS365COM"),
    ("https://theseouljournal.com",    "THESEOULJOURNALCOM"),
]

results = {}

for site, secret_name in SITES:
    pw = os.getenv(secret_name, "")
    entry = {"site": site, "secret": secret_name}
    if not pw:
        entry["error"] = "NO_PW"
        results[site] = entry
        print(f"{site}: ❌ 비밀번호 없음 ({secret_name})")
        sys.stdout.flush()
        continue

    auth = (WP_USER, pw)

    # 1) 모듈 활성화 시도
    module_status = {}
    for mod in ("404-monitor", "redirections"):
        try:
            r = requests.post(
                f"{site}/wp-json/rankmath/v1/saveModule",
                auth=auth,
                json={"module": mod, "state": "on"},
                timeout=15,
            )
            module_status[mod] = r.status_code
        except Exception as e:
            module_status[mod] = f"ERR:{e}"
    entry["module_activation"] = module_status

    # 2) 라우트 재조회
    try:
        r = requests.get(f"{site}/wp-json/rankmath/v1", auth=auth, timeout=15)
        if r.status_code == 200:
            routes = list(r.json().get("routes", {}).keys())
            related = [x for x in routes if "404" in x.lower() or "redirect" in x.lower()]
            entry["related_routes"] = related
        else:
            entry["route_check_error"] = r.status_code
    except Exception as e:
        entry["route_check_error"] = str(e)

    results[site] = entry
    print(f"{site}: 모듈활성화={module_status} 관련라우트={entry.get('related_routes')}")
    sys.stdout.flush()

with open("diag_404_modules_result.json", "w", encoding="utf-8") as f:
    json.dump(results, f, ensure_ascii=False, indent=2)

print("\n완료")
