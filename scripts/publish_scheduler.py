...
# autopost_mega.py의 SITES_CONFIG와 동일한 27개 사이트 URL 목록.
# (무거운 google-genai 등 의존성 없이 순수 requests만 쓰는 이 스케줄러
#  워크플로우에서 autopost_mega.py를 그대로 import하지 않으려고 독립 목록 유지 —
#  다른 스케줄/감사 스크립트들과 동일한 관례)
SITES = [
        ...
        if diff >= 0:
            try:
                r = requests.post(
                    ...
                    json={"ref": "main",
                          "inputs": {"step": "post", "run_slot": "1", "publish_site": site_url}},
                    timeout=20,
                )
