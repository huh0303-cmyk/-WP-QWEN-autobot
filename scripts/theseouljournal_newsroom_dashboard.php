...
add_action('wp_footer', function () {
    ...
    <script id="sj26-newsroom-script">
      document.addEventListener('DOMContentLoaded',()=>{
        ...
        if(document.body.classList.contains('wp-theme-mission-news')){const postDates=<?php echo wp_json_encode($post_dates); ?>,applyBylines=()=>document.querySelectorAll('#loop-container .entry').forEach(entry=>{const match=entry.className.match(/post-(\d+)/),el=entry.querySelector('.post-byline');if(el)el.textContent='SJ편집실 · '+(match&&postDates[match[1]]?postDates[match[1]]:'');});applyBylines();setTimeout(applyBylines,500);setTimeout(applyBylines,1600);}
