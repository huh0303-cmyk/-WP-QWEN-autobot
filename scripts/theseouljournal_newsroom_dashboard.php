...
add_action('wp_footer', function () {
    ...
    <script id="sj26-newsroom-script">
      document.addEventListener('DOMContentLoaded',()=>{
        ...
        if(document.body.classList.contains('wp-theme-mission-news')){const postDates=<?php echo wp_json_encode($post_dates); ?>,applyBylines=()=>document.querySelectorAll('#loop-container .entry').forEach(entry=>{const match=entry.className.match(/post-(\d+)/),el=entry.querySelector('.post-byline'),text='SJ편집실 · '+(match&&postDates[match[1]]?postDates[match[1]]:'');if(el&&el.textContent!==text)el.textContent=text;});applyBylines();const loop=document.querySelector('#loop-container');if(loop)new MutationObserver(applyBylines).observe(loop,{childList:true,subtree:true});}
