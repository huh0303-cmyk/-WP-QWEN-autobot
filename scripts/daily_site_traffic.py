...
def send_to_sheets(records):
    ...
    try:
        ...
        values_by_domain = {r["domain"]: [r["clicks"], r["indexed"]] for r in records}
        # "Youtube-tiktok" 탭과 같은 구조 — A열에 사이트 도메인을 세로로 고정,
        # 날짜가 생길 때마다 오른쪽에 그 날짜용 일일방문자수/색인수 컬럼 추가.
        gsheets_direct.append_dated_metric_columns(
            SHEET_ID, "27개사이트_트래픽", domains, date_label,
            ["일일방문자수", "색인수"], values_by_domain,
        )
        log(f"📊 구글시트 직접 쓰기 완료 ({date_label} 컬럼 추가) — 탭: 27개사이트_트래픽")
    except Exception as e:
