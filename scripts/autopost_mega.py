...
def run_three_model_gate(title, content, meta, keyword):
    """Mandatory Gemini/GPT/Claude checks-and-balances gate (EDITORIAL_AND_MEDIA_MANDATE.md #14).

    Reuses the shared three_model_consensus gate that Blogger already runs,
    driving its independent Gemini self-check through this file's own
    policy-routed generate_content_gemini (never escalates to GPT for a
    check call).
    """
    return three_model_consensus(
        title=title, content=content, meta=meta, keyword=keyword,
        gemini_generate=lambda check: generate_content_gemini(check, use_gpt=False),
    )
