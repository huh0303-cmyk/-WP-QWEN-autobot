...
POST_ID = 1729
OLD_IMAGE_URL = "https://koreamedicaltour.com/wp-content/uploads/2026/09/news-75d6101649df.jpg"

for query in ["korea traditional festival lantern", "korea festival travel", "korea autumn festival street"]:
    r = requests.get("https://pixabay.com/api/", params={
        "key": PIXABAY_KEY, "q": query, "image_type": "photo",
        "orientation": "horizontal", "safesearch": "true", "min_width": 1200, "per_page": 10,
    }, timeout=15)
    r.raise_for_status()
    hits = r.json().get("hits", [])
    if hits:
        print(f"query '{query}' -> {len(hits)} hits, using first")
        photo = hits[0]
        break
else:
    raise SystemExit("no pixabay result for any query")

img = requests.get(photo["largeImageURL"], timeout=30, headers={"User-Agent": "Mozilla/5.0"})
img.raise_for_status()
content_type = img.headers.get("content-type", "image/jpeg")
ext = "png" if "png" in content_type else "jpg"
fname = f"kmt-festival-fix-{photo['id']}.{ext}"
...
print("pixabay source:", photo.get("pageURL"), "by", photo.get("user"))
