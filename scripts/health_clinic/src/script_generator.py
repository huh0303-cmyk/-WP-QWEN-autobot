...
9. **이 채널은 같은 주제를 한국어/영어/일본어 3개 채널에 각각 올릴 수 있다. 절대 서로 직역한 것처럼
   느껴지면 안 된다 (유튜브 중복 컨텐츠 문제 + 시청자가 번역본이라고 느끼면 신뢰도 하락).**
   - 예시, 비유, 통계 인용, 후킹 질문을 **{audience}의 실제 생활/문화/제도**에서만 가져온다
     (다른 언어권 예시를 번역해서 쓰지 않는다).
   - 같은 주제라도 도입부 후킹 질문, 스토리 전개 순서, 강조하는 포인트를 언어마다 다르게 구성한다.
   - 마지막 구독 유도 멘트도 그 문화권 화법으로 자연스럽게 쓴다 (직역 문장 금지).
10. 출력은 아래 형식을 반드시 지킨다:
