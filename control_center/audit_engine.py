...
def _looks_like_public_content(url: str) -> bool:
    ...
    return (
        ...
        and bool(path.strip("/"))
        and not any(x in path for x in ("wp-admin", "wp-login", "preview", "manage/", "dashboard"))
    )


def verify_web_publication(
    public_url: str,
    site_url: str,
    *,
    expected_title: str = "",
    ...
    try:
        response = requests.get(
            ...
            headers={"User-Agent": "Korea365-ProjectA-Auditor/1.0"},
        )
    ...
    if expected_title and not evidence.get("title_match") and marker and not evidence.get("marker_match"):
    ...
    *,
    ...
    if isinstance(receipt, (str, Path)):
        ...
        try:
            ...
            return AuditResult(False, "NEEDS_ATTENTION", "youtube", expected_channel_id, {"receipt": str(path), "error": str(exc)}, "invalid upload receipt")
    else:
        data = dict(receipt)
