(() => {
    ...
    [['전체','전체'],['WordPress','일반 WP 25 · K-Health 포함'],['Blogspot','Blogspot 33'],['뉴스룸','뉴스룸 2 · 각 3~10/일']].forEach(([value,label])=>{const option=document.createElement('option');option.value=value;option.textContent=label;select.append(option);});
    ...
    window.simplifyMetrics=()=>{
        ...
        if(!host.querySelector('.metrics-definitions')){
            const notes=[...host.children].filter(node=>node.tagName==='P'&&!node.classList.contains('metrics-reference-time')&&!node.textContent.startsWith('기준:'));
        ...
        const stamp=[...host.children].find(node=>node.tagName==='P'&&(node.classList.contains('metrics-reference-time')||node.textContent.startsWith('기준:')));
        let freshness=host.querySelector('.metrics-freshness');
        if(stamp){
            if(!freshness){freshness=document.createElement('p');freshness.className='metrics-freshness';stamp.after(freshness);}
            const measured=stamp.dataset.date||stamp.textContent.match(/\d{4}-\d{2}-\d{2}/)?.[0];
            const today=new Intl.DateTimeFormat('en-CA',{timeZone:'Asia/Seoul',year:'numeric',month:'2-digit',day:'2-digit'}).format(new Date());
            freshness.textContent=measured&&measured!==today?'이전 집계 자료입니다. 오늘 통계 갱신을 기다리고 있습니다.':'오늘 집계 자료 · 정확한 확인 시각은 각 항목에서 확인하세요.';
            freshness.style.color=measured!==today?'#b45309':'#047857';
        }
        applyFilter();
    };
    window.simplifyMetrics();
