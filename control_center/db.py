...
class Store:
    ...
    def create_job(self, *, site_id: str, keyword: str) -> dict[str, Any]:
        normalized = " ".join(keyword.lower().split())
        key = f"wordpress:{site_id}:{normalized}:v1"
        stamp = now_iso()
        with self.connect(immediate=True) as con:
            ...
            con.execute(
                "INSERT INTO jobs(id,idempotency_key,site_id,keyword,state,created_at,updated_at) VALUES(?,?,?,?,?,?,?)",
                (job_id, key, site_id, keyword.strip(), CREATED, stamp, stamp),
            )
