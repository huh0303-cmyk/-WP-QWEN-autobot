...
from .models import DEFAULT_IMAGE_MODEL, DEFAULT_TEXT_MODEL, require_image_model, require_text_model
...
class ControlCenter:
    ...
    def create(self, site_id: str, keyword: str, text_model: str = DEFAULT_TEXT_MODEL, image_model: str = DEFAULT_IMAGE_MODEL):
        if site_id not in self.sites:
            raise ValueError("등록되지 않은 WordPress 사이트입니다")
        if len(keyword.strip()) < 3:
            raise ValueError("키워드는 3자 이상 입력하세요")
        return self.store.create_job(site_id=site_id, keyword=keyword, text_model=require_text_model(text_model), image_model=require_image_model(image_model))
    ...
    def generate(self, job_id: str):
        ...
        try:
            article = generate_article(site, job["keyword"], feedback, text_model=job["text_model"])
